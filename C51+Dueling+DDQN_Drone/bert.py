...
input_ids, segment_ids, masked_tokens, masked_pos, isNext = \
    torch.LongTensor(input_ids).to(device),  torch.LongTensor(segment_ids).to(device), torch.LongTensor(masked_tokens).to(device), \
    torch.LongTensor(masked_pos).to(device), torch.LongTensor(isNext).to(device)
...
logits_lm = logits_lm.to('cpu').data.max(2)[1][0].data.numpy()
print('masked tokens list : ',[pos for pos in masked_tokens if pos != 0])
print('predict masked tokens list : ',[pos for pos in logits_lm if pos != 0])

logits_clsf = logits_clsf.cpu().data.max(1)[1].data.numpy()[0]
